...
export function RegisterScreen() {
  const { register, isLoading, loginError, setAuthScreen } = useAuthStore();
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!username || !email || !password || !passwordConfirm || isLoading) return;
    try {
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              disabled={isLoading}
              className="w-full bg-nexus-bg border border-nexus-border rounded px-3 py-2 text-nexus-text font-mono text-sm focus:outline-none focus:border-nexus-cyan/50 focus:ring-1 focus:ring-nexus-cyan/30 disabled:opacity-50 placeholder:text-nexus-text-secondary/40"
              placeholder="analyst@nexus.local"
              autoComplete="email"
            />
          </div>
          ...
          <div>
            ...
            />
            <p className="text-nexus-text-secondary/50 text-[10px] mt-1 font-mono">
          ...
          <div className="text-center pt-1">
            ...
            >
              Already have an account? <span className="text-nexus-cyan">Sign In</span>
            </button>
          </div>
        </form>
